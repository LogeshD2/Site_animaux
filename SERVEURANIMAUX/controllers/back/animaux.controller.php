...
class AnimauxController
{
    ...
    public function visualisation()
    {
        if (Securite::verifAccessSession()) {
            ...
            throw new Exception("Vous n'avez pas le droite d'être là ! ");
        }
    }
    ...
    public function creationValidation()
    {
        if (Securite::verifAccessSession()) {
            ...
            $idAnimal = $this->animauxManager->createAnimaux($nom, $description, $image, $famille);

            $continentManager = new ContinentsManager();
            if (!empty($_POST['continent-1']))
                $continentManager->addContinentAnimal($idAnimal, 1);
            if (!empty($_POST['continent-2']))
                $continentManager->addContinentAnimal($idAnimal, 2);
            if (!empty($_POST['continent-3']))
                $continentManager->addContinentAnimal($idAnimal, 3);
            if (!empty($_POST['continent-4']))
                $continentManager->addContinentAnimal($idAnimal, 4);
            if (!empty($_POST['continent-5']))
                $continentManager->addContinentAnimal($idAnimal, 5);
    ...
    public function modification($idAnimal)
    {
        if (Securite::verifAccessSession()) {
            ...
            $ligneAnimal = $this->animauxManager->getAnimal((int)Securite::secureHTML($idAnimal));
            $tabContinent = [];
            foreach ($ligneAnimal as $continent) {
                $tabContinent[] =   $continent['continent_id'];
            }
            $animal = array_slice($ligneAnimal[0], 0, 5);
