...
class Application extends Component {
    ...
    loadData = () => {
        ...
        axios.get(`http://localhost/React/Site_Animaux/SERVEURANIMAUX/front/animaux/${famille}/${continent}`)
            .then(reponse => {
    ...
    componentDidMount = () => {
        this.loadData();
        axios.get(`http://localhost/React/Site_Animaux/SERVEURANIMAUX/front/continent`)
            .then(reponse => {
                this.setState({listeContinents:Object.values(reponse.data)})
            })
        axios.get(`http://localhost/React/Site_Animaux/SERVEURANIMAUX/front/famille`)
            .then(reponse => {
    ...
    render() {
        ...
        if(this.state.filtreFamille) {
            const numCaseFamilleFiltre = this.state.listeFamilles.findIndex(famille => {
                return parseInt(famille.famille_id) === parseInt(this.state.filtreFamille);
            });
        ...
        if(this.state.filtreContinent) {
            const numCaseContinentFiltre = this.state.listeContinents.findIndex(continent => {
                return parseInt(continent.continent_id) === parseInt(this.state.filtreContinent);
            });
            ...
            <>
                ...
                <div className="container-Fluid">
                    ...
                    <select onChange={(event) => this.handleSelectionContinent(event.target.value)}>
                        <option value="-1" selected={this.state.filtreContinent === null && "selected"}>Continent</option>
                        {
                    ...
                    <div className="row no-gutters">
                        {
                            ...
                            this.state.animaux.map(animal => {
                                return (
                                    <div className="col-12 col-md-6 col-xl4" key={animal.id}>
                                       <Animal 
                                            {...animal} 
